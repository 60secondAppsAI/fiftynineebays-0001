					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateMessage()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Messages = true">Add</base-button>
								              <modal :show.sync="modal2Messages">
								  ...
								  <base-input label="Timestamp" type="text" placeholder="Enter Timestamp" v-model="messageToAdd.timestamp"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Message Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="MessageId" type="text" placeholder="Enter MessageId" v-model="messageDetails.messageId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Content</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Content" type="text" placeholder="Enter Content" v-model="messageDetails.content"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Timestamp</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="messageDetails.timestamp"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateMessage() {
      ...
      const jsonData = JSON.stringify(this.messageDetails);
      const res = await MessageService.update(jsonData);
...
//        this.getAllMessages();
    ...
    async getMessageDetails() {
      try {
			let response = await MessageService.get(this.messageId);

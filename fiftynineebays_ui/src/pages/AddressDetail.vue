					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateAddress()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Addresss = true">Add</base-button>
								              <modal :show.sync="modal2Addresss">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Address Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="AddressId" type="text" placeholder="Enter AddressId" v-model="addressDetails.addressId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Street</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Street" type="text" placeholder="Enter Street" v-model="addressDetails.street"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">City</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="City" type="text" placeholder="Enter City" v-model="addressDetails.city"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Postal Code</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PostalCode" type="text" placeholder="Enter PostalCode" v-model="addressDetails.postalCode"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2Addresss = false;

      const jsonData = JSON.stringify(this.addressToAdd);
      console.log(jsonData);
      
      const res = await AddressService.addAddress(jsonData);
    ...
    async updateAddress() {
      ...
      const jsonData = JSON.stringify(this.addressDetails);
      const res = await AddressService.update(jsonData);
...
//        this.getAllAddresss();
    ...
    async getAddressDetails() {
      try {
			let response = await AddressService.get(this.addressId);

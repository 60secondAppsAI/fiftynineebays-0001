...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Addresss</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAddresss = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAddresss">
                <template slot="header">
  ...
  <base-input label="PostalCode" type="text" placeholder="Enter PostalCode" v-model="addressToAdd.postalCode"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="addresss" :row-key="record => record.AddressId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AddressPictureView :addresss="addresss" />
          </div>
...
import AddressPictureView from './AddressPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} addresss`,
      },
      ...
      sortBy: 'addressId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAddresssTable() {
      ...
      let addresssTableData = [];
      for (let i = 0; i < this.addresss.length; i++) {
        addresssTableData.push({
          id: i,
          addressId: this.addresss[i].addressId,
          year: this.addresss[i].year,
          date: this.addresss[i].date,
          competitionId: this.addresss[i].competitionId,
          addressId: this.addresss[i].addressId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-addresss',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAddresssChanged', this.searchQuery);
		//this.renderAddresssTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.addressToAdd.created = currentDate;

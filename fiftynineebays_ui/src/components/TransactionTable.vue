...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalTransactions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTransactions">
                <template slot="header">
  ...
  <base-input label="TransactionStatus" type="text" placeholder="Enter TransactionStatus" v-model="transactionToAdd.transactionStatus"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="transactions" :row-key="record => record.TransactionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <TransactionPictureView :transactions="transactions" />
          </div>
...
import TransactionPictureView from './TransactionPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'transactionStatus',
          ...
          sorter: true
          //sorter: (a, b) => a.transactionStatus - b.transactionStatus,
          //sorter: (a, b) => a.transactionStatus.localeCompare(b.transactionStatus),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'transactionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTransactionsTable() {
      ...
      for (let i = 0; i < this.transactions.length; i++) {
        transactionsTableData.push({
          id: i,
          transactionId: this.transactions[i].transactionId,
          year: this.transactions[i].year,
          date: this.transactions[i].date,
          competitionId: this.transactions[i].competitionId,
          transactionId: this.transactions[i].transactionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-transactions',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTransactionsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.transactionToAdd.created = currentDate;

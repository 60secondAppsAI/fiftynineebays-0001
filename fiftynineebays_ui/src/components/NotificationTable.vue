        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalNotifications = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalNotifications">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Notification</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="NotificationId" type="text" placeholder="Enter NotificationId" v-model="notificationToAdd.notificationId"></base-input>
  <base-input label="Message" type="text" placeholder="Enter Message" v-model="notificationToAdd.message"></base-input>
  <base-input label="ReceivedDate" type="text" placeholder="Enter ReceivedDate" v-model="notificationToAdd.receivedDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="notifications" :row-key="record => record.NotificationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <NotificationPictureView :notifications="notifications" />
          </div>
...
import NotificationPictureView from './NotificationPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'notificationId',
          visible: true,
          scopedSlots: { customRender: 'notificationId' },
          sorter: true
      ...
      sortBy: 'notificationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderNotificationsTable() {
      ...
      let notificationsTableData = [];
      for (let i = 0; i < this.notifications.length; i++) {
        notificationsTableData.push({
          id: i,
          notificationId: this.notifications[i].notificationId,
          year: this.notifications[i].year,
          date: this.notifications[i].date,
          competitionId: this.notifications[i].competitionId,
          notificationId: this.notifications[i].notificationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-notifications',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForNotificationsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.notificationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.notificationToAdd);
      console.log(jsonData);
      
      const res = await NotificationService.addNotification(jsonData);

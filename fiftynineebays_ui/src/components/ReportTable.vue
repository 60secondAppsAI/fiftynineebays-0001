...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalReports = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalReports">
                <template slot="header">
  ...
  <base-input label="ReportDate" type="text" placeholder="Enter ReportDate" v-model="reportToAdd.reportDate"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="reports" :row-key="record => record.ReportId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ReportPictureView :reports="reports" />
          </div>
...
import ReportPictureView from './ReportPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'reportReason',
          ...
          sorter: true
          //sorter: (a, b) => a.reportReason - b.reportReason,
      ...
      sortBy: 'reportId',           // Column to sort by
  ...
  methods: {
    ...
    async renderReportsTable() {
      ...
      for (let i = 0; i < this.reports.length; i++) {
        reportsTableData.push({
          ...
          year: this.reports[i].year,
          date: this.reports[i].date,
          competitionId: this.reports[i].competitionId,
          reportId: this.reports[i].reportId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-reports',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForReportsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.reportToAdd.created = currentDate;

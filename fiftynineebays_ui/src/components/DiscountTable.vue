...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalDiscounts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDiscounts">
                <template slot="header">
  ...
  <base-input label="ExpirationDate" type="text" placeholder="Enter ExpirationDate" v-model="discountToAdd.expirationDate"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="discounts" :row-key="record => record.DiscountId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DiscountPictureView :discounts="discounts" />
          </div>
...
import DiscountPictureView from './DiscountPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'discountId',
          visible: true,
          scopedSlots: { customRender: 'discountId' },
          sorter: true
          //sorter: (a, b) => a.discountId - b.discountId,
		...
		dataIndex: 'discountCode',
          visible: true,
          scopedSlots: { customRender: 'discountCode' },
          sorter: true
      ...
      sortBy: 'discountId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDiscountsTable() {
      ...
      let discountsTableData = [];
      for (let i = 0; i < this.discounts.length; i++) {
        discountsTableData.push({
          id: i,
          discountId: this.discounts[i].discountId,
          year: this.discounts[i].year,
          date: this.discounts[i].date,
          competitionId: this.discounts[i].competitionId,
          discountId: this.discounts[i].discountId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-discounts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDiscountsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.discountToAdd.created = currentDate;

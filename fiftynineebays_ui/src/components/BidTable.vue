...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBids = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBids">
                <template slot="header">
  ...
  <base-input label="BidDate" type="text" placeholder="Enter BidDate" v-model="bidToAdd.bidDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="bids" :row-key="record => record.BidId" :pagination="pagination"
...
import BidPictureView from './BidPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'bidId',
          visible: true,
          scopedSlots: { customRender: 'bidId' },
          sorter: true
          //sorter: (a, b) => a.bidId - b.bidId,
		...
		dataIndex: 'bidDate',
          visible: true,
          scopedSlots: { customRender: 'bidDate' },
          sorter: true
          //sorter: (a, b) => a.bidDate - b.bidDate,
          //sorter: (a, b) => a.bidDate.localeCompare(b.bidDate),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'bidId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBidsTable() {
      ...
      for (let i = 0; i < this.bids.length; i++) {
        bidsTableData.push({
          id: i,
          bidId: this.bids[i].bidId,
          year: this.bids[i].year,
          date: this.bids[i].date,
          competitionId: this.bids[i].competitionId,
          bidId: this.bids[i].bidId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-bids',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBidsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalBids = false;

      const currentDate = new Date().getTime();
      this.bidToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.bidToAdd);
      console.log(jsonData);
      
      const res = await BidService.addBid(jsonData);

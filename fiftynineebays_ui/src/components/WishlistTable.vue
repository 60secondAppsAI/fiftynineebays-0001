        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalWishlists = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalWishlists">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Wishlist</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="WishlistId" type="text" placeholder="Enter WishlistId" v-model="wishlistToAdd.wishlistId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="wishlistToAdd.name"></base-input>
  <base-input label="CreatedDate" type="text" placeholder="Enter CreatedDate" v-model="wishlistToAdd.createdDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="wishlists" :row-key="record => record.WishlistId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <WishlistPictureView :wishlists="wishlists" />
          </div>
...
import WishlistPictureView from './WishlistPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'wishlistId',
          visible: true,
          scopedSlots: { customRender: 'wishlistId' },
          sorter: true
          //sorter: (a, b) => a.wishlistId - b.wishlistId,
          //sorter: (a, b) => a.wishlistId.localeCompare(b.wishlistId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} wishlists`,
      },
      ...
      sortBy: 'wishlistId',           // Column to sort by
  ...
  methods: {
    ...
    async renderWishlistsTable() {
      ...
      let wishlistsTableData = [];
      for (let i = 0; i < this.wishlists.length; i++) {
        wishlistsTableData.push({
          id: i,
          wishlistId: this.wishlists[i].wishlistId,
          year: this.wishlists[i].year,
          date: this.wishlists[i].date,
          competitionId: this.wishlists[i].competitionId,
          wishlistId: this.wishlists[i].wishlistId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-wishlists',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForWishlistsChanged', this.searchQuery);
		//this.renderWishlistsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalWishlists = false;

      const currentDate = new Date().getTime();
      this.wishlistToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.wishlistToAdd);
      console.log(jsonData);
      
      const res = await WishlistService.addWishlist(jsonData);

...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalUsers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalUsers">
                <template slot="header">
  ...
  <base-input label="ContactNumber" type="text" placeholder="Enter ContactNumber" v-model="userToAdd.contactNumber"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="users" :row-key="record => record.UserId" :pagination="pagination"
...
import UserPictureView from './UserPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'userId',           // Column to sort by
  ...
  methods: {
    ...
    async renderUsersTable() {
      ...
      for (let i = 0; i < this.users.length; i++) {
        usersTableData.push({
          ...
          year: this.users[i].year,
          date: this.users[i].date,
          competitionId: this.users[i].competitionId,
          userId: this.users[i].userId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-users',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForUsersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.userToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.userToAdd);
      console.log(jsonData);
      
      const res = await UserService.addUser(jsonData);

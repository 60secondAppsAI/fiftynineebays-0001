        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalOffers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalOffers">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Offer</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="OfferId" type="text" placeholder="Enter OfferId" v-model="offerToAdd.offerId"></base-input>
  <base-input label="OfferAmount" type="text" placeholder="Enter OfferAmount" v-model="offerToAdd.offerAmount"></base-input>
  <base-input label="OfferStatus" type="text" placeholder="Enter OfferStatus" v-model="offerToAdd.offerStatus"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="offers" :row-key="record => record.OfferId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <OfferPictureView :offers="offers" />
          </div>
...
import OfferPictureView from './OfferPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'offerAmount',
          visible: true,
          scopedSlots: { customRender: 'offerAmount' },
          sorter: true
          //sorter: (a, b) => a.offerAmount - b.offerAmount,
          //sorter: (a, b) => a.offerAmount.localeCompare(b.offerAmount),
        },
        {
          title: 'Offer Status',
		dataIndex: 'offerStatus',
          visible: true,
          scopedSlots: { customRender: 'offerStatus' },
          sorter: true
          //sorter: (a, b) => a.offerStatus - b.offerStatus,
      ...
      sortBy: 'offerId',           // Column to sort by
  ...
  methods: {
    ...
    async renderOffersTable() {
      ...
      for (let i = 0; i < this.offers.length; i++) {
        offersTableData.push({
          id: i,
          offerId: this.offers[i].offerId,
          year: this.offers[i].year,
          date: this.offers[i].date,
          competitionId: this.offers[i].competitionId,
          offerId: this.offers[i].offerId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-offers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForOffersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalOffers = false;

      const currentDate = new Date().getTime();
      this.offerToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.offerToAdd);
      console.log(jsonData);
      
      const res = await OfferService.addOffer(jsonData);

        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalSubscriptions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSubscriptions">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Subscription</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="SubscriptionId" type="text" placeholder="Enter SubscriptionId" v-model="subscriptionToAdd.subscriptionId"></base-input>
  <base-input label="Plan" type="text" placeholder="Enter Plan" v-model="subscriptionToAdd.plan"></base-input>
  <base-input label="SubscriptionDate" type="text" placeholder="Enter SubscriptionDate" v-model="subscriptionToAdd.subscriptionDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="subscriptions" :row-key="record => record.SubscriptionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SubscriptionPictureView :subscriptions="subscriptions" />
          </div>
...
import SubscriptionPictureView from './SubscriptionPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'subscriptionId',
          visible: true,
          scopedSlots: { customRender: 'subscriptionId' },
          sorter: true
          //sorter: (a, b) => a.subscriptionId - b.subscriptionId,
          //sorter: (a, b) => a.subscriptionId.localeCompare(b.subscriptionId),
        },
        {
          title: 'Plan',
		dataIndex: 'plan',
		...
		dataIndex: 'subscriptionDate',
          visible: true,
          scopedSlots: { customRender: 'subscriptionDate' },
          sorter: true
          //sorter: (a, b) => a.subscriptionDate - b.subscriptionDate,
      ...
      sortBy: 'subscriptionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSubscriptionsTable() {
      ...
      let subscriptionsTableData = [];
      for (let i = 0; i < this.subscriptions.length; i++) {
        subscriptionsTableData.push({
          id: i,
          subscriptionId: this.subscriptions[i].subscriptionId,
          year: this.subscriptions[i].year,
          date: this.subscriptions[i].date,
          competitionId: this.subscriptions[i].competitionId,
          subscriptionId: this.subscriptions[i].subscriptionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-subscriptions',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSubscriptionsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalSubscriptions = false;

      const currentDate = new Date().getTime();
      this.subscriptionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.subscriptionToAdd);
      console.log(jsonData);
      
      const res = await SubscriptionService.addSubscription(jsonData);

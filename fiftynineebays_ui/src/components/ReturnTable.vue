        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalReturns = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalReturns">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Return</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ReturnId" type="text" placeholder="Enter ReturnId" v-model="returnToAdd.returnId"></base-input>
  <base-input label="Reason" type="text" placeholder="Enter Reason" v-model="returnToAdd.reason"></base-input>
  <base-input label="ReturnStatus" type="text" placeholder="Enter ReturnStatus" v-model="returnToAdd.returnStatus"></base-input>
  <base-input label="InitiatedDate" type="text" placeholder="Enter InitiatedDate" v-model="returnToAdd.initiatedDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="returns" :row-key="record => record.ReturnId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ReturnPictureView :returns="returns" />
          </div>
...
import ReturnPictureView from './ReturnPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'returnId',
          visible: true,
          scopedSlots: { customRender: 'returnId' },
          sorter: true
          //sorter: (a, b) => a.returnId - b.returnId,
		...
		dataIndex: 'returnStatus',
          ...
          sorter: true
          //sorter: (a, b) => a.returnStatus - b.returnStatus,
		...
		dataIndex: 'initiatedDate',
          visible: true,
          scopedSlots: { customRender: 'initiatedDate' },
          sorter: true
          //sorter: (a, b) => a.initiatedDate - b.initiatedDate,
          //sorter: (a, b) => a.initiatedDate.localeCompare(b.initiatedDate),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'returnId',           // Column to sort by
  ...
  methods: {
    ...
    async renderReturnsTable() {
      ...
      let returnsTableData = [];
      for (let i = 0; i < this.returns.length; i++) {
        returnsTableData.push({
          id: i,
          returnId: this.returns[i].returnId,
          year: this.returns[i].year,
          date: this.returns[i].date,
          competitionId: this.returns[i].competitionId,
          returnId: this.returns[i].returnId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-returns',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForReturnsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalReturns = false;

      const currentDate = new Date().getTime();
      this.returnToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.returnToAdd);
      console.log(jsonData);
      
      const res = await ReturnService.addReturn(jsonData);

...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPromotions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPromotions">
                <template slot="header">
  ...
  <base-input label="PromotionType" type="text" placeholder="Enter PromotionType" v-model="promotionToAdd.promotionType"></base-input>
  <base-input label="PromotionStartDate" type="text" placeholder="Enter PromotionStartDate" v-model="promotionToAdd.promotionStartDate"></base-input>
  <base-input label="PromotionEndDate" type="text" placeholder="Enter PromotionEndDate" v-model="promotionToAdd.promotionEndDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="promotions" :row-key="record => record.PromotionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PromotionPictureView :promotions="promotions" />
          </div>
...
import PromotionPictureView from './PromotionPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'promotionId',
          visible: true,
          scopedSlots: { customRender: 'promotionId' },
          sorter: true
		...
		dataIndex: 'promotionType',
          ...
          sorter: true
          //sorter: (a, b) => a.promotionType - b.promotionType,
          //sorter: (a, b) => a.promotionType.localeCompare(b.promotionType),
        },
        {
          title: 'Promotion Start Date',
		dataIndex: 'promotionStartDate',
          visible: true,
          scopedSlots: { customRender: 'promotionStartDate' },
          sorter: true
          //sorter: (a, b) => a.promotionStartDate - b.promotionStartDate,
          //sorter: (a, b) => a.promotionStartDate.localeCompare(b.promotionStartDate),
        },
        {
          title: 'Promotion End Date',
		dataIndex: 'promotionEndDate',
          visible: true,
          scopedSlots: { customRender: 'promotionEndDate' },
          sorter: true
          //sorter: (a, b) => a.promotionEndDate - b.promotionEndDate,
          //sorter: (a, b) => a.promotionEndDate.localeCompare(b.promotionEndDate),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'promotionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPromotionsTable() {
      ...
      let promotionsTableData = [];
      for (let i = 0; i < this.promotions.length; i++) {
        promotionsTableData.push({
          id: i,
          promotionId: this.promotions[i].promotionId,
          year: this.promotions[i].year,
          date: this.promotions[i].date,
          competitionId: this.promotions[i].competitionId,
          promotionId: this.promotions[i].promotionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-promotions',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPromotionsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalPromotions = false;

      const currentDate = new Date().getTime();
      this.promotionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.promotionToAdd);
      console.log(jsonData);
      
      const res = await PromotionService.addPromotion(jsonData);

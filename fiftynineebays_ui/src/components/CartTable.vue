        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalCarts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCarts">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Cart</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CartId" type="text" placeholder="Enter CartId" v-model="cartToAdd.cartId"></base-input>
  <base-input label="Quantity" type="text" placeholder="Enter Quantity" v-model="cartToAdd.quantity"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="carts" :row-key="record => record.CartId" :pagination="pagination"
...
import CartPictureView from './CartPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'cartId',
          visible: true,
          scopedSlots: { customRender: 'cartId' },
          sorter: true
          //sorter: (a, b) => a.cartId - b.cartId,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} carts`,
      },
      ...
      sortBy: 'cartId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCartsTable() {
      ...
      for (let i = 0; i < this.carts.length; i++) {
        cartsTableData.push({
          id: i,
          cartId: this.carts[i].cartId,
          year: this.carts[i].year,
          date: this.carts[i].date,
          competitionId: this.carts[i].competitionId,
          cartId: this.carts[i].cartId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-carts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCartsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalCarts = false;

      const currentDate = new Date().getTime();
      this.cartToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.cartToAdd);
      console.log(jsonData);
      
      const res = await CartService.addCart(jsonData);
